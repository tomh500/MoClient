...
if /I "%CURRENT_FOLDER_NAME%" neq "%EXPECTED_FOLDER_NAME%" (
    ...
    echo 错误: 你的 DearMoments 放置位置错误， 请重新查看教程的安装指引
    pause
    exit /b 1
)
...
:: 检查最后一行是否是 "exec DearMoments/Setup"
if /i not "!LAST_LINE_CONTENT!"=="exec DearMoments/Setup" (
    if /i not "!LAST_LINE_CONTENT!"=="exec DearMoments/Setup " (
        echo Last line is not "exec DearMoments/Setup". Adding it to the file...
        echo.  >> "%AUTOEXEC_FILE%" 
        echo exec DearMoments/Setup >> "%AUTOEXEC_FILE%"
    ) else (
        echo Last line is already "exec DearMoments/Setup". No changes made.
    )
) else (
    echo Last line is already "exec DearMoments/Setup". No changes made.
)
